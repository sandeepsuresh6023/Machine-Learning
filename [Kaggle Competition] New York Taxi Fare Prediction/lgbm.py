...
# Reading Data
train_df =  pd.read_csv('train_1.csv', nrows = 100000)
...
train_df['fare-bin'] = pd.cut(train_df['fare_amount'], bins = list(range(0, 50, 5))).astype(str)
train_df.loc[train_df['fare-bin'] == 'nan', 'fare-bin'] = '[45+]'
train_df.loc[train_df['fare-bin'] == '(5, 10]', 'fare-bin'] = '(05, 10]'
...
x_train,x_test,y_train,y_test = train_test_split(train_df,y,stratify = train_df['fare-bin'],random_state=123,test_size=8000)
x_train = x_train.drop(columns=['fare-bin'])
x_test = x_test.drop(columns=['fare-bin'])
y_train = y_train.drop(columns=['fare-bin'])
y_test = y_test.drop(columns=['fare-bin'])
